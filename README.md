<div align="center">
  <img src="frontend/img/icon.png" alt="VersePC-CE Logo" width="120">
  <h1>VersePC-CE</h1>
...
**VersePC-CE** 是 [VersePC2](https://github.com/doujie081231/VersePc2) 的社区版（Community Edition）。VersePC2 是 VersePC 的 Tauri 重写版，用 Rust 后端 + Web 前端搭建的 Minecraft 启动器。相比 Electron 版，它更轻量、启动更快、内存占用更低，同时保留了完整的功能。
...
- **下载管理器**：高速多线程下载引擎，Java、模组、整合包、任意文件统一走下载任务
- **壁纸引擎**：自定义背景、视频壁纸、全景主题
- **AI 助手**：内置多种 AI 提供商，支持工具调用（Agent 化）
- **文件浏览器**：内置文件管理
- **TTS 语音合成**
- **Dino 小游戏**：离线时的小彩蛋
...
│   │   ├── frp/         # FRP 内网穿透（LoliaFrp / OpenFrp / SakuraFrp）
│   │   ├── easytier/    # 陶瓦联机 / 内网穿透
│   │   ├── network/     # 网络（UPnP / 公网 IP）
│   │   ├── server_query.rs  # 测试服务器（Server List Ping，含 SRV 解析）
│   │   ├── java.rs      # Java 检测与安装
│   │   ├── storage.rs   # 数据持久化
│   │   └── system.rs    # 环境检查（WebView2）
│   ├── resources/       # 内嵌资源（frpc 客户端等）
│   ├── icons/           # 应用图标
...
### 打包
```bash
npm run build          # 便携版（生成 dist/VersePC-CE.exe）
