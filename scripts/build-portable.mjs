...
//    正常产物名是 Cargo 包名 verse-tauri.exe；若 Tauri CLI 按 productName 重命名了二进制
//    （tauri.conf.json → productName，当前为 VersePC-CE），也一并兼容，避免打包脚本误报找不到产物。
const productExeName = `${tauriConf.productName || 'VersePC-CE'}.exe`;
const exeSrc = firstExisting([
  ...
  join(cargoTargetDir, 'release', productExeName),
  join(projectRoot, 'src-tauri', 'target', 'release', productExeName),
]);
if (!exeSrc) {
  console.error(`[build-portable] 错误：找不到编译产物 verse-tauri.exe / ${productExeName}`);
  process.exit(1);
}
...
// 打包产物统一为固定名 VersePC-CE.exe，不带版本后缀；
// 升级时更新器把新包覆盖到正在运行的 exe 文件名上，使安装后的 exe 始终是 VersePC-CE.exe。
copyFileSync(exeSrc, join(portableDir, 'VersePC-CE.exe'));

// 5. 复制运行所需的配套 DLL
//    注：tauri v2 已静态链接 WebView2Loader，exe 不再引用该 DLL（实测 target 产物导入表无此项）。
//    若仓库里碰巧有就一并带上（兼容旧 GNU 构建方式），没有也不阻断打包。
...
if (dllSrc) {
  ...
  console.log('[build-portable] 未找到 WebView2Loader.dll（tauri v2 静态链接，通常无需携带，继续打包）');
}

// 5.5 复制 tauri resources（运行时从 exe 目录解析：resources/frp/frpc.exe 等）
const resourcesSrc = join(projectRoot, 'src-tauri', 'resources');
if (existsSync(resourcesSrc)) {
  for (const entry of readdirSync(resourcesSrc, { recursive: true, withFileTypes: true })) {
    const rel = join(entry.parentPath ?? entry.path, entry.name).slice(resourcesSrc.length + 1);
    if (entry.isDirectory()) {
      mkdirSync(join(portableDir, rel), { recursive: true });
    } else {
      mkdirSync(join(portableDir, rel, '..'), { recursive: true });
      copyFileSync(join(resourcesSrc, rel), join(portableDir, rel));
    }
  }
  console.log(`[build-portable] 已复制 resources: ${resourcesSrc}`);
}
...
console.log(`[build-portable] 产物: VersePC-CE.exe`);
