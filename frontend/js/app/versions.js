...
async function navigateToPage(pageName) {
  ...
  if (pageName === 'assistant') {
    setTimeout(() => {
      try {
        if (window.VerseAssistantPage && typeof window.VerseAssistantPage.refresh === 'function') {
          window.VerseAssistantPage.refresh();
        }
      } catch (e) { console.warn('[Verse助手] 进入页面刷新失败:', e); }
    }, 60);
  }
  ...
  // 切到账户页：先把轨道藏起来（resetCarouselPosition 一进来就置 pending，并自带重试），
  // 再在页面可见、尺寸稳定后定位到中间。必须立刻调一次 —— 否则从页面可见到 350ms 之间
  // 轨道会以未定位的状态露出来，等后面再纠正就是一次肉眼可见的位移。
