      ...
      showImportNameModal(fileBaseName, function(customName) {
        ...
        window.electronAPI.importModpack(filePath, customName).then(result => {
          ...
          if (result && result.success) {
            if (typeof dlManager !== 'undefined') { dlManager.update(taskId, { status: 'completed', progress: 100, message: '导入完成' }); }
            showToast(`整合包 "${result.name || '未知'}" 导入成功！`, 'success');
          } else {
            var errMsg = result?.error || '未知错误';
            if (typeof dlManager !== 'undefined') { dlManager.update(taskId, { status: 'error', message: errMsg }); }
            showToast(`导入失败: ${errMsg}`, 'error');
          }
        }).catch(err => {
          window._modpackImporting = false;
          var catchMsg = err.message || '';
          if (typeof dlManager !== 'undefined') { dlManager.update(taskId, { status: 'error', message: catchMsg }); }
          showToast('导入失败: ' + catchMsg, 'error');
        });
      });
    }
  }
