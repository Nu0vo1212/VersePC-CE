...
function _getTranslateAIConfig() {
  // V 岛已删除：AI 配置唯一事实源是 Verse 助手（assistant.js，键 verse-ai-config）。
  // 优先走它的 getConfig()（内部含旧键迁移），读不到再直接读 localStorage 兜底。
  try {
    if (window.VerseAssistant && typeof window.VerseAssistant.getAIConfig === 'function') {
      const c = window.VerseAssistant.getAIConfig();
      if (c && Object.keys(c).length) return c;
    }
  } catch (_) {}
  try { return JSON.parse(localStorage.getItem('verse-ai-config') || localStorage.getItem('v-island-ai-config') || '{}'); }
  catch (_) { return {}; }
}
...
// 上面这份是早期子集。完整供应商表（含 Kimi / 豆包 / 硅基流动 …）在
// Verse 助手（assistant.js）的 AI_PROVIDERS 里，优先复用它。
function _resolveAIProvider(id) {
  try {
    const full = window.VerseAssistant && window.VerseAssistant.AI_PROVIDERS;
    if (full && full[id]) return full[id];
  } catch (_) {}
  return _AI_PROVIDERS[id] || null;
}
...
async function startAutoTranslate() {
  ...
  const aiConfig = _getTranslateAIConfig();
  if (!aiConfig || !aiConfig.provider || !aiConfig.apiKey) {
    showToast('请先在「Verse 助手」页的设置里配置 AI 供应商', 'error');
    return;
  }
...
async function _translateWithAI(entries, aiConfig, isJson, onProgress) {
  ...
  const providerInfo = _resolveAIProvider(provider);
  const baseConfig = {
    provider: provider,
