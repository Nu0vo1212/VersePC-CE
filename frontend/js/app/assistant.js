...
(function () {
  ...
  var SUGGESTIONS = [
    ...
    { label: '装个整合包', q: '我想装个 1.20.1 的整合包，有什么推荐的？' },
  ...
  function buildSystemPrompt() {
    return [
      ...
      '',
      '【装模组 / 整合包（游戏版本必须先定清楚）】',
      '- 装模组或整合包之前，**先确定装到哪个游戏版本**，这是必须先问清的信息：',
      '  · 用户已经说了版本（「帮我装 1.20.1 的 XX」「XX 整合包 1.20.1」）→ 直接用这个版本号，不要再问。',
      '  · 用户没说 → **先用一句话问清楚要哪个游戏版本**，不要自己随便挑一个。',
      '  · 用户不确定有什么版本 → 先调 list_resource_versions 查出该整合包/模组支持哪些游戏版本（只读，立刻执行），把可选的版本列给用户挑。',
      '- 安装时把游戏版本填进 install_modpack 的 gameVersion、或 install_mod 的 version 参数：系统会按这个版本挑对应的构建文件，装出来的就是那个版本的。',
      '- 该资源确实**没有**用户要的版本时，工具会直接报「没有适配游戏版本 X 的文件」——照实告诉用户，并把实际可选的版本列出来让用户改选，不要自己换成别的版本。',
      '- 搜不到就换平台再搜：Modrinth 和 CurseForge 的内容不一样，source 留空表示两边一起搜；同一次回答里不要反复搜同一个词。',
      '',
      '【回答风格】',
      '- 中文；先给结论，再给步骤；简洁、可操作。',
