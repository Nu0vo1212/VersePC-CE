...
async function init() {
  ...
  try {
    // 主题（黑色 / 白色 / 自定义）：统一走 applySavedTheme()，
    // 数据源是 store 单项键 versepc_theme —— 与个性化页保存、loadSettings() 共用同一条链路，
    // 避免"启动早期一套、loadSettings 又一套"互相覆盖导致主题来回跳。
    ...
    if (confirmed) {
      try {
        ...
        if (result.success) {
          ...
          // 重置后回到内置主题，清掉自定义主题遗留的 inline 变量，
          // 让 --accent 回落到 themes.css 的白色主题值（#1a1a1a），不要 inline 写死。
