 ...
 * Tauri 兼容的 /api 请求包装。
 *
 * Tauri 下没有本地 HTTP 服务器，裸 fetch('/api/...') 必然失败 —— 这正是工具箱
 * 「下载自定义文件」此前一按就报错的原因。所有 /api 调用必须走 apiProxy（invoke）
 * 通道；非 Tauri 环境（旧浏览器壳）仍回退原生 fetch。
 */
async function _customDlFetch(url, options = {}) {
    // accounts.js 已提供同语义的实现，能复用就复用，避免两份逻辑漂移
    if (typeof _tauriFetch === 'function') {
...
function openCustomDlFolder() {
    ...
    if (p) {
        _customDlFetch('/api/open-folder', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ folder: 'custom', customPath: p })
        }).catch(() => {});
    } else {
...
async function startCustomDownload() {
    ...
    try {
        const res = await _customDlFetch('/api/download-custom', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        console.error('[custom-download] 创建下载任务失败:', e);
        showToast('下载请求失败: ' + (e && e.message ? e.message : e), 'error');
        resetCustomDlUI();
    }
}
...
function pollCustomDlProgress() {
    ...
    _customDlPollTimer = setInterval(async () => {
        ...
        try {
            const res = await _customDlFetch(`/api/download-custom/status?sessionId=${encodeURIComponent(_customDlSessionId)}`);
            const data = await res.json();
            if (data.status === 'not_found') { clearInterval(_customDlPollTimer); resetCustomDlUI(); return; }

            // 页面切换后元素可能已不在 DOM 中，逐项做空值保护，避免轮询抛错
            const fillEl = document.getElementById('custom-dl-progress-fill');
            const textEl = document.getElementById('custom-dl-progress-text');
            const statusEl = document.getElementById('custom-dl-status');
            ...
            if (data.status === 'completed') {
                ...
                clearInterval(_customDlPollTimer);
                showToast(data.message || '下载失败', data.status === 'cancelled' ? 'info' : 'error');
                resetCustomDlUI();
            }
        } catch (e) {
            console.error('[custom-download] 查询进度失败:', e);
        }
    }, 500);
}
...
async function cancelCustomDownload() {
    if (_customDlSessionId) {
        try {
            await _customDlFetch('/api/download-custom/cancel', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ sessionId: _customDlSessionId })
            });
        } catch (e) {
            console.error('[custom-download] 取消下载失败:', e);
        }
    }
    if (_customDlPollTimer) clearInterval(_customDlPollTimer);
    resetCustomDlUI();
...
function resetCustomDlUI() {
    ...
    const startBtn = document.getElementById('custom-dl-start-btn');
    const cancelBtn = document.getElementById('custom-dl-cancel-btn');
