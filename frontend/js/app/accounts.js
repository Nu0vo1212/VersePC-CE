...
function _carouselResetPosition(animate) {
  ...
  const target = _carouselTargetX();

  // 容器还没有真实宽度（账户页未激活 / 布局未完成）时不要落位置：
  // 否则会先写入一个错的 translateX（此时必为 -cardW/2，轨道偏在左边），
  // 等页面可见后再纠正 —— 那次纠正就是肉眼可见的「瞬移」。
  // 不落位时保持「无 transform」，交由 resetCarouselPosition() 在尺寸就绪后统一定位。
  if (target !== null) {
...
// 「揭开轨道」的令牌：定位可能被连着调用好几次（进入账户页有 350/450/700/900ms 多次），
// 只有最后一次有资格移除 pending，否则早先那次的回调会把还没画出来的新位置提前揭开。
let _carouselRevealToken = 0;
...
function resetCarouselPosition() {
  ...
  const ctx = document.getElementById('accts-carousel');
  const token = ++_carouselRevealToken;

  let hidden = false;
  const hide = () => {
    if (hidden || !ctx) return;
    ctx.classList.add('accts-carousel-pending');
    hidden = true;
  };

  // 位置还没定过：页面一可见就先藏，别以未定位的样子露（这种情况必然要落一次位置）
  if (!trackEl.style.transform) hide();

  // 揭开时机必须按「位置到底有没有变」来决定：
  //   位置没变 —— 同帧收起隐藏即可，浏览器只画最终状态，不会闪（所以「没要变就不藏」）；
  //   位置变了 —— 必须等新位置真正画过一帧再揭开，否则隐藏等于没藏，跳变照旧看得见。
  // 注意：只有最后一次调用有资格收尾（进入账户页会在 350/450/700/900ms 被连着调多次），
  // 并且收尾时无条件清掉 pending —— 否则早先那次藏起来后，它的回调已被令牌作废，
  // 就再也没人去揭开，轨道会一直隐身。
  const reveal = (frames) => {
    requestAnimationFrame(() => {
      if (token !== _carouselRevealToken) return; // 已有更新的定位在跑，交给它收尾
      if (frames <= 1) { if (ctx) ctx.classList.remove('accts-carousel-pending'); return; }
      reveal(frames - 1);
    });
  };
  ...
  const tryCenter = () => {
    const first = trackEl.querySelector('.accts-card');
    const ready = ctx && first && ctx.clientWidth > 0 && first.offsetWidth > 0;
    if (ready || retries >= 60) {
      const before = trackEl.style.transform;
      const target = _carouselTargetX();
      // 明确知道位置要变，才在这一帧先藏起来（同帧隐藏 + 改位，浏览器只画最终状态：
      // 隐藏 + 新位置），之后两帧再淡入
      if (target !== null && `translateX(${target}px)` !== before) hide();
      _carouselResetPosition(false);
      setTimeout(() => { trackEl.style.transition = ''; }, 0);
      const changed = trackEl.style.transform !== before;
      reveal(changed ? 2 : 1);
      return;
    }
    retries += 1;
