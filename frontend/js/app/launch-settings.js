...
function toggleAdvancedOptions() {
  ...
  if (!content) return;
  if (content._advAnimating) return;
  ...
  if (isHidden) {
    // 展开：display 后从 0 → 实际高度过渡
    content.style.display = 'block';
    const fullH = content.scrollHeight;
    content.style.overflow = 'hidden';
    content.style.height = '0px';
    content.style.opacity = '0';
    void content.offsetHeight; // 强制回流，让起始值生效
    content.style.transition = `height ${DUR}ms ${EASE}, opacity ${DUR}ms ${EASE}`;
    content.style.height = fullH + 'px';
    content.style.opacity = '1';
    content._advAnimating = true;
    setTimeout(() => {
      content._advAnimating = false;
...
async function resetLaunchSettings() {
  ...
  document.getElementById('launch-custom-info').value = 'VersePC-CE';
...
async function applyThemeByName(theme) {
  ...
  if (isCustom) {
    ...
    // 内置主题（黑色 / 白色）：只清掉自定义主题留下的 inline 变量，不再自己写 --accent。
    // --accent / --accent-hover / --accent-rgb / --accent-glow 全部由 themes.css 按
    // [data-theme="dark"]（#ffffff）与 [data-theme="light"]（#1a1a1a）提供 —— 单一数据源，
    // 这样"黑色永远黑、白色永远白"，不会被上一次的自定义色或历史遗留的 accentColor 污染。
