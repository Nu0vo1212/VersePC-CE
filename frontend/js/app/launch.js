...
function showCrashAnalysisDialog(result, versionId) {
  ...
  overlay.style.cssText = 'position:fixed;top:0;left:0;right:0;bottom:0;z-index:10001;display:flex;align-items:center;justify-content:center;background:rgba(0,0,0,0.6);animation:crashFadeIn 0.2s ease';
  ...
  // "让 Verse 助手帮我修"按钮：跳到助手页并预填崩溃信息，由 AI 调用后端工具给出修复建议
  const vfixBtn = dialog.querySelector('#crash-dialog-vfix');
  if (vfixBtn) {
    vfixBtn.addEventListener('click', () => {
      closeDialog();
      const logLines = result.logExcerpt ? result.logExcerpt.slice(0, 10).map(i => i.line).join('\n') : '';
      const prompt = '我的 Minecraft 启动崩溃了，帮我分析原因并给出修复方案。\n'
        + '崩溃原因: ' + (result.reason || '未知') + '\n'
        + (result.modName ? '相关模组: ' + result.modName + '\n' : '')
        + (result.solution ? '初步建议: ' + result.solution + '\n' : '')
        + (result.logFile ? '日志文件: ' + result.logFile + '\n' : '')
        + (logLines ? '错误日志摘要:\n' + logLines : '');
      if (typeof navigateToPage === 'function') navigateToPage('assistant');
      setTimeout(() => {
        try {
          const page = window.VerseAssistantPage;
          if (page && typeof page.ask === 'function') {
            page.ask(prompt);
          } else if (page) {
            page.input = prompt;
          } else {
            showToast('Verse 助手暂不可用', 'warning');
          }
        } catch (e) {
          showToast('跳转 Verse 助手失败: ' + (e.message || e), 'error');
        }
      }, 300);
    });
  }
