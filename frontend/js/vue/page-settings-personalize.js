 ...
 *   2. ThemeAppearanceCard / BackgroundCard 两个卡片子组件
...
(function () {
  ...
  const MotionCard = {
    ...
    methods: {
      onSpeedInput() {
        if (typeof applyAnimationSpeedValue === 'function') applyAnimationSpeedValue(this.state.animSpeed);
      }
    },
    template: `
          <div class="card">
            <h3>动效</h3>
            <div class="form-group">
              <label>动画速度 <span class="custom-theme-color-value">&times;{{ Number(state.animSpeed).toFixed(2) }}</span></label>
              <input type="range" min="0.5" max="2" step="0.05" class="wallpaper-slider" v-model.number="state.animSpeed" @input="onSpeedInput">
              <span class="form-hint">控制全站所有界面动画与过渡的速度：大于 1.0 变快，小于 1.0 变慢，1.00 为标准速度。调整立即生效。</span>
            </div>
          </div>
  `
  };
