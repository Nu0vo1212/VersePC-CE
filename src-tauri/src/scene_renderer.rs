...
fn renderer_package_github_url() -> String {
    format!(
        "https://github.com/Nu0vo1212/VersePC-CE/releases/download/v{}/{}",
        env!("CARGO_PKG_VERSION"),
        RENDERER_PACKAGE_FILE
    )
}

// ============== 下载源构建（原 updater.rs 的公共逻辑，更新器移除后收编至此） ==============
...
/// 把 release 下载地址展开为完整下载源列表：GitHub 优先，最后 ghfast 等镜像。
fn build_download_sources(url: &str) -> Vec<String> {
    ...
    if url.starts_with("https://gitee.com/") {
        ...
        let gh = format!(
            "https://github.com/Nu0vo1212/VersePC-CE/releases/download/{}/{}",
            tag, file
        );
...
pub async fn scene_renderer_download(app: tauri::AppHandle) -> Value {
    ...
    // Gitee → GitHub → ghfast 镜像
    let sources = build_download_sources(&renderer_package_github_url());
