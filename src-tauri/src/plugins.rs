...
/// 校验本地插件包路径：必须存在且为 .zip。
fn validate_local_zip(path: &str) -> Result<PathBuf, String> {
    let p = path.trim();
    if p.is_empty() {
        return Err("未选择插件包".to_string());
    }
    ...
    let is_zip = pb
        ...
        .unwrap_or(false);
    if !is_zip {
        return Err("仅支持 .zip 格式的插件包".to_string());
    }
    Ok(pb)
}

/// 读取本地 zip 内的 plugin.json（**不安装**），用于安装前预览清单与权限确认。
/// plugin.json 允许位于 zip 根目录或单层顶层子目录（与 install_from_zip 一致）。
pub async fn inspect_local_zip(path: &str) -> Value {
    use std::io::Read;

    let pb = match validate_local_zip(path) {
        Ok(p) => p,
        Err(e) => return json!({ "ok": false, "error": e }),
    };
    let file = match std::fs::File::open(&pb) {
        Ok(f) => f,
        Err(e) => return json!({ "ok": false, "error": format!("无法打开插件包: {}", e) }),
    };
    let mut archive = match zip::ZipArchive::new(file) {
        Ok(a) => a,
        Err(e) => return json!({ "ok": false, "error": format!("插件包不是有效的 zip: {}", e) }),
    };

    // 定位 plugin.json：根目录优先，其次单层子目录
    ...
    for i in 0..archive.len() {
        ...
        if sub_idx.is_none() {
            ...
            if parts.len() == 2 && parts[1] == "plugin.json" {
                sub_idx = Some(i);
            }
        }
    }
    let idx = match root_idx.or(sub_idx) {
        Some(i) => i,
        None => return json!({ "ok": false, "error": "插件包内未找到 plugin.json" }),
    };
    ...
    let mut content = String::new();
    {
        let mut entry = match archive.by_index(idx) {
            Ok(e) => e,
            Err(e) => return json!({ "ok": false, "error": format!("读取 plugin.json 失败: {}", e) }),
        };
        if let Err(e) = entry.read_to_string(&mut content) {
            return json!({ "ok": false, "error": format!("读取 plugin.json 失败: {}", e) });
        }
    }

    let m: Value = match serde_json::from_str(&content) {
        Ok(v) => v,
        Err(e) => return json!({ "ok": false, "error": format!("plugin.json 不是合法 JSON: {}", e) }),
    };
    let id = m.get("id").and_then(|v| v.as_str()).unwrap_or("");
    let name = m.get("name").and_then(|v| v.as_str()).unwrap_or("");
    let version = m.get("version").and_then(|v| v.as_str()).unwrap_or("");
    if id.is_empty() || name.is_empty() || version.is_empty() {
        return json!({ "ok": false, "error": "插件清单缺少 id / name / version" });
    }
    json!({ "ok": true, "manifest": m, "zipPath": pb.to_string_lossy() })
}

/// 安装本地插件包：复用 install_from_zip 的解压 / 防穿越 / 落地 / 索引更新逻辑。
/// 与市场安装的差别只有两点：包来自本地文件；不做 sha256 校验（离线包无市场哈希）。
pub async fn install_local(path: String) -> Value {
    if let Err(e) = validate_local_zip(&path) {
        return json!({ "ok": false, "error": e });
    }
    install_from_zip(path.trim().to_string(), String::new()).await
}
...
/// 预览本地插件包（读取清单，不安装）。请求体：{ path }
#[tauri::command]
pub async fn plugin_local_inspect(path: String) -> Value {
    inspect_local_zip(&path).await
}

/// 从本地 zip 安装插件。请求体：{ path }
#[tauri::command]
pub async fn plugin_install_local(path: String) -> Value {
    install_local(path).await
}
